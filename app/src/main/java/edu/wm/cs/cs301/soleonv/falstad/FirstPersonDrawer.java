...
import edu.wm.cs.cs301.soleonv.generation.BSPBranch;
import edu.wm.cs.cs301.soleonv.generation.BSPLeaf;
import edu.wm.cs.cs301.soleonv.generation.BSPNode;
...
import edu.wm.cs.cs301.soleonv.generation.Seg;
import edu.wm.cs.cs301.soleonv.generation.Wall;
...
public class FirstPersonDrawer extends DefaultViewer {
	...
	private NewMazePanel mazepanel;
	 ...
	 */
	@Override
	public void redraw(NewMazePanel mp, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
	 ...
	 * @param gc
	 */
	private void drawBackground(NewMazePanel mazepanel) {
		int[] purple = new int[]{75,0,130};
		mazepanel.setColor(purple);
		mazepanel.fillRect(0, 0, view_width, view_height/2);
		int[] gold = new int[]{255,90,0};
		mazepanel.setColor(gold);
