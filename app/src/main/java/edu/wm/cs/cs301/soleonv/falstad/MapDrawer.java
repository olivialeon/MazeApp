...
public class MapDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(NewMazePanel mp, StateGUI state, int px, int py,
		...
		if (null != controller) {
			if (controller.isInMapMode()) {
				draw_map(mp, px, py, walk_step, view_dx, view_dy, controller.isInShowMazeMode(), controller.isInShowSolutionMode()) ;
				draw_currentlocation(mp, view_dx, view_dy) ;
			}
		}
	}
	 ...
	 */
	public void draw_map(NewMazePanel mazepanel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		// dimensions of the maze
		final int mazew = mazeConfig.getWidth() ;
		final int mazeh = mazeConfig.getHeight() ;
		
		mazepanel.setGraphicsWhite();
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				//mazepanel.setColor(seencells.hasWall(x,y, CardinalDirection.North) ? Color.white : Color.gray);
				if (seencells.hasWall(x,y, CardinalDirection.North))
					mazepanel.setGraphicsWhite();
				else
					mazepanel.setGraphicsGray();
				...
				//mazepanel.setColor(seencells.hasWall(x,y, CardinalDirection.West) ? Color.white : Color.gray);
	 ...
	 * @param gc to draw on
	 */
	public void draw_currentlocation(NewMazePanel mazepanel, int view_dx, int view_dy) {
		mazepanel.setGraphicsRed();
	 ...
	 * @param offy
	 */
	public void draw_solution(NewMazePanel mazepanel, int offx, int offy, int px, int py) {
