...
import edu.wm.cs.cs301.soleonv.generation.Factory;
...
import edu.wm.cs.cs301.soleonv.ui.GeneratingActivity;
 ...
 */
public class MazeController extends Application implements Order {
	...
	private RobotDriver Driver; //selected driver

	enum DriverList {Wizard, Manual, WallFollower, Pledge}
	...
	public MazeController(Context context) {
		...
		panel = new NewMazePanel(context);
		...
		switch (key) {
			case ('w' & 0x1f): // Ctrl-w makes a step forward even through a wall
				if (mazeConfig.isValidPosition(px + dx, py + dy)) {
					setCurrentDirection(px + dx, py + dy);
					notifyViewerRedraw();
				}
				break;
			case '\t':
			case '1': // show local information: current position and visible walls
				// precondition for showMaze and showSolution to be effective
				// acts as a toggle switch
				mapMode = !mapMode;
				notifyViewerRedraw();
				break;
			case '3': // show a map of the whole maze
				// acts as a toggle switch
				showMaze = !showMaze;
				notifyViewerRedraw();
				break;
			case '5': // show the solution on the map as a yellow line towards the exit
				// acts as a toggle switch
	...
	public void setBuilder(String builder) {
		...
		else if (builder.equalsIgnoreCase("Eller"))
			this.builder = Builder.Eller;
	}
	...
	public void setPanel(NewMazePanel panel){
		this.panel = panel;
	}
	...
	void setDriver(DriverList driver){
		this.showMaze = true;
	...
	public void deliver(MazeConfiguration mazeConfig) {
		...
		cleanViews();
		addView(new FirstPersonDrawer(Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT, Constants.MAP_UNIT, Constants.STEP_SIZE, seencells, mazeConfig.getRootnode()));
		addView(new MapDrawer(Constants.VIEW_WIDTH, Constants.VIEW_WIDTH, Constants.MAP_UNIT,Constants.STEP_SIZE, seencells, 10, this));
		gen.toPlayScreen();
	}
	/**
